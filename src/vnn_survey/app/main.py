...
from vnn_survey.app.pipeline_service import PipelineService, test_openai_connection
...
def _render_scope_page(store: ProjectStore, project: ProjectSettings) -> None:
    ...
    try:
        ...
        st.caption(
            _t(
                "The grouped expression compiles to {count} DBLP request(s).",
                count=len(queries),
            )
        )
        with st.expander(_t("Show DBLP requests")):
...
def _render_ai_settings(store: ProjectStore, project: ProjectSettings) -> None:
    ...
    st.write(
        _t("Configure the model used to screen abstracts. Final decisions always remain manual.")
    )
    left, right = st.columns(2)
    with left:
        model = st.text_input(
            _t("Model"), value=project.llm_model, key=f"ai_model_{project.slug}"
        )
    with right:
